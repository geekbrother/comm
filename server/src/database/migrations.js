...
async function getDBVersion(): Promise<number> {
  ...
  try {
    ...
    const getDBVersionExceptionMessage = String(getMessageForException(e));
    console.error(`(node:${process.pid}) getDBVersion() failed.`);
    console.error(getDBVersionExceptionMessage);
    return 0;
  }
}
