...
function ThreadAncestors(props: ThreadAncestorsProps): React.Node {
  ...
  const threadColorStyle = React.useMemo(() => ({ color: `#${threadColor}` }), [
    threadColor,
  ]);
  ...
  const keyserverInfo = React.useMemo(
    ...
    [community.uiName, keyserverOwnerUsername, threadBackgroundColorStyle],
  );
  ...
  }, [threadHasNoAncestors, threadBackgroundColorStyle, threadInfo.uiName]);
