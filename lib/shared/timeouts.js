...
// To detect this happening, the client hits the server with a "ping" at interval
// specified below when there hasn't been any other communication.
export const pingFrequency = 3000; // in milliseconds

// Time for request to get response after which we consider our
// connection questionable. We won't close and reopen the socket yet, but we
...
// client. If server doesn't get a request after time window specified
// below, it will close connection
export const serverRequestSocketTimeout = 120000; // in milliseconds

// Time server allows itself to respond to client message. If it takes it
// longer to respond, it will timeout and send an error response. This is better
