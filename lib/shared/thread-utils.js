...
export type SelectedThreadColors = $Keys<typeof selectedThreadColors>;

function generateRandomColor(): string {
  const potentialColors = Object.keys(selectedThreadColors);
...
function generatePendingThreadColor(userIDs: $ReadOnlyArray<string>): string {
  ...
  const colorIdx = stringHash(ids) % potentialColors.length;
  return potentialColors[colorIdx];
}
