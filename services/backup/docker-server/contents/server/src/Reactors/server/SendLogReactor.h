...
SendLogReactor::readRequest(backup::SendLogRequest request) {
  ...
  switch (this->state) {
    case State::USER_ID: {
      ...
      std::shared_ptr<database::BackupItem> lastBackupItem =
          database::DatabaseManager::getInstance().findLastBackupItem(
              this->userID);
      if (lastBackupItem == nullptr) {
        throw std::runtime_error(
            "last backup for user " + this->userID + " not found");
      }
      this->backupID = lastBackupItem->getBackupID();
      std::cout << "retreived last backup from the DB, backup id: "
                << this->backupID << std::endl;
