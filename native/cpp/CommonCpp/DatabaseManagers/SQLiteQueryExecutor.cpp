...
void validate_encryption() {
  ...
  if (temp_encrypted_exists && default_location_exists) {
    Logger::log(
        "Deleting unencrypted database failed in previous encryption attempt. "
        "Repeating delete and rename steps.");
    attempt_delete_file(
  ...
  if (!key_validation_error) {
    Logger::log(
        "Database exists under default path and it is correclty encrypted.");
    return;
  }
