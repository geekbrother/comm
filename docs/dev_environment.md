...
These lines are different depending on if you're using an Apple Silicon Mac or an Intel Mac. If you're on Apple Silicon, you should add the following to your desired shell configuration file (note the addition of the `--no-use` flag on the line that loads nvm):
...
Otherwise, for Intel Macs, add the following to your desired shell configuration file:
